...
// Nommé `allTraffic` et non `global` : `const global` masquerait l'objet
// global de Node dans tout ce module — un piège pour la prochaine édition.
const allTraffic = redis
  ? new Ratelimit({
      redis,
...
export async function checkRateLimit(ip, limiters = null) {
  ...
  const globalLimiter = limiters?.global ?? allTraffic
